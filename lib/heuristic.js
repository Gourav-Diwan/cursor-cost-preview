...
// provider's public docs as of 2026-08-04. THESE CHANGE OFTEN — if that date
...
const MODELS = {
  ...
  opus:   { label: 'Claude Opus 5',     provider: 'Anthropic', inPM: 5,  outPM: 25 },
  fable:  { label: 'Claude Fable 5 (Mythos 5)', provider: 'Anthropic', inPM: 10, outPM: 50 },
  // OpenAI — 5.4/5.5 non-Pro tiers refreshed into the 5.6 Luna/Terra/Sol
  // lineup as of this update; GPT-5.5 Pro is untouched, still the top tier.
  'gpt-luna':  { label: 'GPT-5.6 Luna',  provider: 'OpenAI', inPM: 0.2, outPM: 1.2 },
  'gpt-terra': { label: 'GPT-5.6 Terra', provider: 'OpenAI', inPM: 2,   outPM: 12  },
  'gpt-sol':   { label: 'GPT-5.6 Sol',   provider: 'OpenAI', inPM: 5,   outPM: 30  }, // flagship
  ...
  'gemini-flash': { label: 'Gemini 3.6 Flash',      provider: 'Google', inPM: 1.5,  outPM: 7.5 },
  ...
  // Microsoft AI — MAI-Code-1-Flash is the only MAI text model with
  // confirmed public per-token pricing; Microsoft's other "MAI" models are
  // mostly Copilot-bundled or not independently priced.
  'mai-code-flash': { label: 'MAI-Code-1-Flash', provider: 'Microsoft AI', inPM: 0.75, outPM: 4.5 },
  // Perplexity — Sonar family. These are search-augmented models; routing
  // capability below leans on that specialty rather than treating them as
  // generic chat models.
  sonar:                { label: 'Sonar',                provider: 'Perplexity', inPM: 1, outPM: 1  },
  'sonar-pro':           { label: 'Sonar Pro',            provider: 'Perplexity', inPM: 3, outPM: 15 },
  'sonar-reasoning-pro': { label: 'Sonar Reasoning Pro',  provider: 'Perplexity', inPM: 2, outPM: 8  },
...
const MODEL_ORDER = [
  'llama-8b', 'llama-scout', 'grok-fast', 'llama-maverick', 'gpt-luna',
  'gemini-lite', 'sonar', 'grok', 'mai-code-flash', 'haiku', 'gemini-flash',
  'sonar-reasoning-pro', 'sonnet', 'gemini-pro', 'gpt-terra', 'sonar-pro',
  'opus', 'gpt-sol', 'fable', 'gpt-pro',
];
...
// Perplexity's Sonar family is search-augmented, not general-purpose — scoped
// to research/code/other rather than ALL_TYPES (no strong creative-writing
// claim). MAI-Code-1-Flash is scoped to code/other per its name — no
// evidence of general-purpose strength beyond that.
const MODEL_CAPABILITY = {
  ...
  'gpt-luna':       { maxBucketIdx: 3, types: ['email', 'creative', 'other'] },
  'gemini-lite':    { maxBucketIdx: 2, types: ['email', 'creative', 'other'] },
  sonar:            { maxBucketIdx: 3, types: ['research', 'email', 'other'] },
  grok:             { maxBucketIdx: 5, types: ALL_TYPES },
  'mai-code-flash':     { maxBucketIdx: 4, types: ['code', 'other'] },
  haiku:                { maxBucketIdx: 2, types: ['email', 'creative', 'other'] },
  'gemini-flash':       { maxBucketIdx: 4, types: ALL_TYPES },
  'sonar-reasoning-pro': { maxBucketIdx: 5, types: ['research', 'code', 'other'] },
  sonnet:           { maxBucketIdx: 5, types: ALL_TYPES },
  'gemini-pro':     { maxBucketIdx: 6, types: ALL_TYPES },
  'gpt-terra':      { maxBucketIdx: 5, types: ALL_TYPES },
  'sonar-pro':      { maxBucketIdx: 6, types: ['research', 'code', 'other'] },
  opus:             { maxBucketIdx: 6, types: ALL_TYPES },
  'gpt-sol':        { maxBucketIdx: 6, types: ALL_TYPES },
