...
// Deep-session extrapolation correction (W11). logPriorTokens (see
// featurizeTask above) feeds a tanh-activated hidden layer; once a real
// session's cumulative depth goes far beyond anything in the training
// data, the hidden units this feature drives are already saturated near
// ±1 at the edge of the training range, so the prediction flatlines
// instead of continuing to scale with depth — see CLAUDE.md's "deep-
// session underestimate" note. Fix is a SECOND, independent calibration
// (same "compute at train time, apply at inference, leave the frozen net
// itself untouched" pattern as sessionSumCalibration below), not a change
// to runTokenModel() or the trained weights themselves:
//   - W.priorTokensExtrap = { trainMaxLog, slope, n }, computed by
//     scripts/train-token-model.cjs as a weighted-least-squares fit of
//     (actual - predicted) vs. logPriorTokens on the upper half of
//     observed training depths, clamped to a non-negative slope <= 1.5.
//   - trainMaxLog is the literal max logPriorTokens seen in that training
//     run — the correction is an EXACT no-op (byte-identical output) for
//     any turn within the training range, and only engages beyond it.
//   - The correction grows as log(1 + excess), not linearly in excess, so
//     it damps out for sessions far beyond training depth rather than
//     compounding — a session 10x beyond training gets far less than 10x
//     the correction a session 2x beyond gets. This is deliberate: a
//     linear correction would risk overshooting the NEXT, even-deeper
//     session the same way the pre-W7 arithmetic-mean pricing bug
//     initially undershot before sessionSumCalibration was added.
// Exposed as a function, not applied inline, for the same vm.runInContext
// reason sessionSumCalibration() is a function — but here it's also just
// cleaner to unit-test/call directly with an explicit W.
function priorTokensExtrapCorrection(logTok, priorTokens, W) {
  const cal = W && W.priorTokensExtrap;
  if (!cal || !(cal.slope > 0)) return logTok; // old/absent weights: no-op
  const L = Math.log(1 + (priorTokens || 0));
  const excess = L - cal.trainMaxLog;
  if (excess <= 0) return logTok; // in-range: exactly unchanged
  return logTok + cal.slope * Math.log(1 + excess);
}
...
function nnEstimateTask(text, modelKey = 'sonnet', opts = {}) {
  ...
  const [rawLogTok, outShareLogit] = runTokenModel(featurizeTask(text, opts), W);
  // Deep-session extrapolation correction (see priorTokensExtrapCorrection
  // below) — a no-op for every in-range prediction, only adjusts logTok
  // when this turn's priorTokens goes beyond what the model was trained on.
  const logTok = priorTokensExtrapCorrection(rawLogTok, opts.priorTokens, W);
  // Clamp — tiny-data extrapolation guardrail: no real turn is under ~100
  // tokens or over a few million. Checked (W11): priorTokensExtrapCorrection
  // above is damped enough (log(1+excess), slope capped at 1.5) that it
  // doesn't come close to this ceiling even at 200M simulated priorTokens
  // (~490k corrected tokens vs. the 6e6 clamp) — the correction is real but
  // modest by design, not large enough to need this raised. Re-check if the
  // slope cap or fit approach ever changes.
